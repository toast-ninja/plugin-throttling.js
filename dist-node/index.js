...
function throttling(octokit, octokitOptions = {}) {
  ...
  state.retryLimiter.on("failed", async function (error, info) {
    ...
    const {
      maxRetryAfterSeconds = Number.MAX_SAFE_INTEGER
    } = info.args[0] || {};
    ...
    const {
      ...
      if (error.headers != null && error.headers["x-ratelimit-remaining"] === "0") {
        ...
        const retryAfter = originalRetryAfter > maxRetryAfterSeconds ? maxRetryAfterSeconds : originalRetryAfter;
  ...
  octokit.hook.after('request', (response, options) => {
    ...
    if (shouldRetry) {
      throw new HttpError(response.statusText, response.status, response.headers);
    }
  });
